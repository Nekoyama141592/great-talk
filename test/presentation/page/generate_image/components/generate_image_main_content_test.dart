...
void main() {
  group('GenerateImageMainContent', () {
    ...
    group('UI Structure', () {
      testWidgets('should render main components without image', (WidgetTester tester) async {
      ...
      testWidgets('should render main components with image', (WidgetTester tester) async {
      ...
      testWidgets('should have proper spacing between components', (WidgetTester tester) async {
        ...
        expect(sizedBoxes.any((box) => box.height == 24), isTrue); // Between prompt and size selector
        expect(sizedBoxes.any((box) => box.height == 32), isTrue); // Between size selector and button, and after button
      });

      testWidgets('should have proper spacing with image', (WidgetTester tester) async {
        const base64 = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==';
    ...
    group('Prompt Input Interactions', () {
      testWidgets('should trigger onPromptChanged callback', (WidgetTester tester) async {
        await tester.pumpWidget(createTestWidget());
        ...
        final modernPromptInputWidget = tester.widget<ModernPromptInput>(promptInput);
        modernPromptInputWidget.onChanged('Test prompt');

        expect(promptChanged, equals('Test prompt'));
      });

      testWidgets('should handle null prompt text', (WidgetTester tester) async {
        await tester.pumpWidget(createTestWidget());
      ...
      testWidgets('should pass correct controller to prompt input', (WidgetTester tester) async {
    ...
    group('Size Selector Interactions', () {
      testWidgets('should display size selector wrapper', (WidgetTester tester) async {
        await tester.pumpWidget(createTestWidget());
      ...
      testWidgets('should pass correct selected size', (WidgetTester tester) async {
        const testSize = '1024x1024';
        
        await tester.pumpWidget(createTestWidget(selectedSizeOverride: testSize));

        // The size should be passed to the internal wrapper
        expect(find.byType(SizeSelector), findsOneWidget);
      });

      testWidgets('should handle size changes through dropdown', (WidgetTester tester) async {
        await tester.pumpWidget(createTestWidget());
        ...
        final horizontalOption = find.text(GenerateImageEnum.horizontal.text()).last;
    ...
    group('Generate Button Interactions', () {
      testWidgets('should trigger onGenerate callback when pressed', (WidgetTester tester) async {
        await tester.pumpWidget(createTestWidget());
      ...
      testWidgets('should handle multiple generate button presses', (WidgetTester tester) async {
        int generateCount = 0;
    ...
    group('Image Display', () {
      testWidgets('should not show image container when base64 is null', (WidgetTester tester) async {
        await tester.pumpWidget(createTestWidget(base64: null));

        expect(find.byType(GlassmorphicImageContainer), findsNothing);
      });

      testWidgets('should show image container when base64 is provided', (WidgetTester tester) async {
        const base64 = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==';
      ...
      testWidgets('should show image container with empty base64', (WidgetTester tester) async {
        const base64 = '';
    ...
    group('Layout and Responsiveness', () {
      ...
      testWidgets('should maintain component order', (WidgetTester tester) async {
        const base64 = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8/5+hHgAHggJ/PchI7wAAAABJRU5ErkJggg==';
    ...
    group('State Management', () {
      testWidgets('should update when selectedSize changes', (WidgetTester tester) async {
        await tester.pumpWidget(createTestWidget(selectedSizeOverride: GenerateImageEnum.sqare.text()));

        expect(find.byType(SizeSelector), findsOneWidget);

        // Rebuild with different size
        await tester.pumpWidget(createTestWidget(selectedSizeOverride: GenerateImageEnum.horizontal.text()));

        expect(find.byType(SizeSelector), findsOneWidget);
      });

      testWidgets('should update when base64 changes', (WidgetTester tester) async {
        await tester.pumpWidget(createTestWidget(base64: null));
    ...
    group('Edge Cases', () {
      testWidgets('should handle very long prompt text', (WidgetTester tester) async {
        ...
        expect(find.byType(ModernPromptInput), findsOneWidget);
        final promptInput = tester.widget<ModernPromptInput>(find.byType(ModernPromptInput));
        expect(promptInput.controller.text, equals(longText));
      });

      testWidgets('should handle all available size options', (WidgetTester tester) async {
        for (final sizeEnum in GenerateImageEnum.values) {
          await tester.pumpWidget(createTestWidget(selectedSizeOverride: sizeEnum.text()));
    ...
    group('Integration', () {
      testWidgets('should work as part of larger widget tree', (WidgetTester tester) async {
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
