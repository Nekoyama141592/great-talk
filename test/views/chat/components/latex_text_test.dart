...
void main() {
  group('LatexText', () {
    testWidgets('should display GptMarkdown with white text style', (WidgetTester tester) async {
      const testData = 'Test markdown content';
      
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: LatexText(data: testData),
          ),
        ),
      );
    ...
    testWidgets('should handle different markdown content', (WidgetTester tester) async {
      final testCases = [
        'Simple text',
        '# Heading',
        '**Bold text**',
    ...
    testWidgets('should work with different themes', (WidgetTester tester) async {
      ...
      final lightGptMarkdown = tester.widget<GptMarkdown>(find.byType(GptMarkdown));
      expect(lightGptMarkdown.style?.color, Colors.white);

      // Test with dark theme
      await tester.pumpWidget(buildWithTheme(ThemeData.dark()));
      final darkGptMarkdown = tester.widget<GptMarkdown>(find.byType(GptMarkdown));
      expect(darkGptMarkdown.style?.color, Colors.white);
    });

    testWidgets('should handle multi-line content', (WidgetTester tester) async {
      const multiLineData = '''
# Title
...
''';
      ...
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: LatexText(data: multiLineData),
          ),
        ),
      );
    ...
    testWidgets('should work in different layout contexts', (WidgetTester tester) async {
      ...
      final gptMarkdowns = tester.widgetList<GptMarkdown>(find.byType(GptMarkdown));
      expect(gptMarkdowns.elementAt(0).data, 'First content');
      expect(gptMarkdowns.elementAt(1).data, 'Second content');
    });

    testWidgets('should handle special characters and LaTeX', (WidgetTester tester) async {
      const latexData = r'''
Math formula: $E = mc^2$
...
''';
    ...
    testWidgets('should maintain consistent behavior across rebuilds', (WidgetTester tester) async {
      ...
      final initialGptMarkdown = tester.widget<GptMarkdown>(find.byType(GptMarkdown));
      ...
      final updatedGptMarkdown = tester.widget<GptMarkdown>(find.byType(GptMarkdown));
